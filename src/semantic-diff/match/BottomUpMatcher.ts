...
export default class BottomUpMatcher<T> implements IMatcher<T> {
    ...
    match (oldTree: TNode<T>,
        newTree: TNode<T>,
        comparator: IComparator<T>): void {

        let moreMatches = oldTree.leaves()
            .filter(n => n.isMatched() && !n.isRoot());
        while (moreMatches.length > 0) {
            ...
            for (const node of moreMatches) {
                ...
                if (!cand.isMatched() &&
                    !candMatch.isMatched() &&
                    comparator.compareContent(cand, candMatch) <
                    this.options.COMPARISON_THRESHOLD) {
                    ...
                    for (let i = 0; i < cand.children.length; i++) {
                        if (!cand.childAt(i)
                            .isMatchedTo(candMatch.childAt(i))) {
                            allMatching = false;
                            break;
                        }
                    }
