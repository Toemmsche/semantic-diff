...
export class Comparator<T> extends Cache<T> implements IComparator<T> {
    ...
    compareCommonality (nodeA: TNode<T>, nodeB: TNode<T>): number {
        ...
        for (const cand of setA) {
            if (cand.isMatched() && setB.has(cand.getMatch())) {
                common++;
            }
        }
     ...
     */
    weightedAverage (items: Nullable<number>[],
        weights: number[],
        ...
        for (let i = 0; i < items.length; i++) {
            if (items[i] != null) {
                ...
                const adjustedWeight = (items[i] === 0
                    ? this.options.WEIGHT_BOOST_MULTIPLIER
                    : 1) * weights[i];
