...
export default function PlanPicker(props: IQueryPlanResultDiffProps) {
        ...
        obj.otherResults
          .map((qpr) => {
            if (!qpr.benchmarkResult.total || !obj.baseLineResult.benchmarkResult.total) {
              ...
              const diff = baseLineMetric / otherMetric - 1;
  ...
  function QueryComponent(props: {}) {
    ...
    const QueryRadios = worstResultsPerQuery.map((obj) => {
      ...
      return (
        <Stack key={query} direction="row" justifyContent="start" alignItems="center">
          <Radio
            key={query}
    ...
    return (
      <Stack direction="column" height="100%" justifyContent="space-between">
        ...
        <Stack direction="row" alignItems="center">
          <IconButton onClick={(event) => setListAnchorEl(event.currentTarget)}>
            <Edit />
          </IconButton>
  ...
  function BaselineComponent(props: {}) {
    ...
    const BaselineRadios = availableDbms.map((dbms) => {
      return <FormControlLabel key={dbms} value={dbms} control={<Radio />} label={dbms} />;
    });
      ...
      <Stack direction="column" height="100%" justifyContent="space-between">
        ...
        <Stack direction="row">
          ...
          <Stack textAlign="center" alignItems="center" justifyContent="center">
            <Chip color="primary" label={baselineDbms ?? 'Please select a baseline'}></Chip>
          </Stack>
        </Stack>
      </Stack>
    );
    ...
    const CompCandidateItems = qprForSelectedQuery
      ...
      .map((qpr) => {
        ...
        if (baseLineQprForSelectedQuery) {
          const diff =
            baseLineQprForSelectedQuery.benchmarkResult.total! / qpr.benchmarkResult.total! - 1;
    ...
    <Stack display="flex" justifyContent="center" alignItems="center">
      <Stack direction="column">
        <Stack direction="row" spacing={50}>
          <Stack key="first" direction="row" alignItems="center" spacing={2}>
